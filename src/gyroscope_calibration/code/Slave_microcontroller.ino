...
void loop() {
  ...
  if (bluetooth.available()) {
    ...
    if (data.length() > 0) {
      processAccelData(data);           // Parse and print the gyroscope values
...
void processAccelData(String data) {
  float values[6];                       // Store parsed float values (X, Y, Z)
  int index = 0;
  int lastIndex = 0;
  ...
  Serial.print(values[3], 3); Serial.print(",");// gyroscope x,y,z axis data
  Serial.print(values[4], 3); Serial.print(",");
  Serial.println(values[5], 3);
}
